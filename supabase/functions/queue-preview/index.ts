...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Cache: se dentro do TTL e não há ticket em atendimento diferente do anterior, retornar direto
    if (Date.now() - previewCache.ts < 3000 && previewCache.payload) {
    ...
    // Recupera apenas uma prévia limitada (por exemplo 30 posições)
    ...
    const result: (Ticket & { pos: number })[] = [];
    let iN = 0,
      iP = 0;
